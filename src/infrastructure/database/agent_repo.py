...
Provides asynchronous CRUD operations for Agent entities.
"""
...
class AgentRepository(BaseRepository[Agent]):
    ...
    agent = await agent_repo.get_by_name("FakeNewsAgent")
    
    # 異步創建 Agent
    new_agent = await agent_repo.create_agent(
    ...
    @with_session
    async def get_by_name(self, agent_name: str, db: Optional[AsyncSession] = None) -> Optional[Agent]:
        """
        ...
        """
        results = await self.get_by(db=db, agent_name=agent_name)
    ...
    async def create_agent(self, 
                           ...
                           db: Optional[AsyncSession] = None, **kwargs) -> Agent:
        """
        創建新的 Agent 實體。
        ...
        # 調用父類的 create 方法
        return await self.create(agent_data, db=db)
